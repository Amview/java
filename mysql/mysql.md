...
2. mysql导入数据csv
* 出于安全考虑，默认是不允许从client host远程通过load data命令导数据。可以登录时候加 ``--local-infile[=1]`` 显式指定参数

```
mysql --local-infile -u root -p 
```

* 服务端和本地端都要开启local_infile
...
//客户端，jdbc需添加allowLoadLocalInfile=true
...
UPDATE stu set age=23,name='张三';
```
* 删除表

```
//清空表的数据，删除数据(删除整行)，不加where会删除表中所有记录
delete from table_name (where);
//删除整个表
drop table table_name;
//删除表中所有数据，包括auto_increment
...
```
//修改表数据
alter table table_name set age=23,name='张三' where id =1;
//添加字段
alter table table_name add field_name field_type;
//修改字段
alter table table_name change id id int not null auto_increment primary key;
//删除字段
alter tabel table_name drop field_name;
```
...
* 模糊查询
```
select * from 表名 where 列名 like ‘张_’; _任意字符，%代表匹配0~n个字符
```
* 排序
```
